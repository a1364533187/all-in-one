...
public class EquationsPossible990Solution {

    public boolean equationsPossible(String[] equations) {
        UnionFindSet unionFindSet = new UnionFindSet(26);
        for (int i = 0; i < equations.length; i++) {
            if (equations[i].charAt(1) == '=') {
                unionFindSet.unite(equations[i].charAt(0) - 'a', equations[i].charAt(3) - 'a');
            }
        }

        for (int i = 0; i < equations.length; i++) {
            if (equations[i].charAt(1) == '!') {
                if (unionFindSet.find(equations[i].charAt(0) - 'a', equations[i].charAt(3) - 'a')) {
                    return false;
                }
            }
        }
